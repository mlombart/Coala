...
nbins_ref=100
kernel_ref = 'k_brownian'
path_data_ref = '../data/' + kernel_ref
...
plt.title(r'Brownian kernel, $\tau=%.2f$'%(tend))
plt.tight_layout()


# to save plot
# plt.savefig('./' + kernel + '_mass_density_distribution.pdf',**savefig_options)


plt.figure(2)

plt.loglog(massmeanlog_ref,massmeanlog_ref*gt0_massmeanlog_ref,'--',c='C0',alpha=0.5,zorder=1)
plt.loglog(massmeanlog_ref,massmeanlog_ref*gtend_massmeanlog_ref,'--',c='C0',label='Ref, k=%d, %d bins'%(dict_data_ref[first_key_ref]['kpol'],nbins_ref),zorder=2)
...
# to save plot
# plt.savefig('./' + kernel + '_mxg_distribution.pdf',**savefig_options)
