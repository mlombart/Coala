...
module iterate_coag_DG
   use precision
   use setup, only:kernel,K0,eps,coeff_CFL,optimised_solver,isave
...
!! @param[in]    massbins      geometric mean value of massgrid for each mass bins
...
subroutine iterate_coag(path_data,path_log,chnbins,nbins,kpol,massgrid,massbins,massmeanlog,Q,vecnodes,vecweights,ndthydro,dthydro)
   implicit none
   ...
   real(wp),          intent(in) :: massgrid(nbins+1),massbins(nbins),massmeanlog(nbins)
   real(wp),          intent(in) :: vecnodes(Q),vecweights(Q)

   integer  :: iprogress,j,k,i,tot_nsub,tot_ndt,nsub,ndt
   real(wp) :: tab_gamma(nbins),g_massmeanlog(nbins),g_val
   ...
   call cpu_time(start)
   gij= 0._wp
   call L2proj_GQ(eps,nbins,kpol,massgrid,massbins,mat_coeffs_leg,Q,vecnodes,vecweights,gij)



   !apply scaling limiter on gij to ensure positivity
   call gammafunction(eps,nbins,kpol,massgrid,massbins,mat_coeffs_leg,gij,tab_gamma)
   do j=1,nbins
      do k=1,kpol
         gij(j,k+1) = tab_gamma(j)*gij(j,k+1)
      enddo
   enddo

   !check gij values and limit to eps value
   do j=1,nbins
   ...
   do i=1,ndthydro
      ...
      if (optimised_solver) then
         call compute_coag_opt(eps,coeff_CFL,nbins,kpol,massgrid,massbins,mat_coeffs_leg,&
                                 tabflux_coag,tabintflux_coag,&
                                 dthydro,gij,gijnew,nsub,ndt)

      else
         call compute_coag(eps,coeff_CFL,nbins,kpol,massgrid,massbins,mat_coeffs_leg,&
                                 tabflux_coag,tabintflux_coag,&
                                 dthydro,gij,gijnew,nsub,ndt)
      endif
   ...
   if (isave) then
      open(unit=1,file=trim(path_log),action='write',position='append')
      write(1,'("Initial total dust mass density ",e30.16E3,"s")') M1_t0
      write(1,'("Final total dust mass density ",e30.16E3,"s")') M1_tend
