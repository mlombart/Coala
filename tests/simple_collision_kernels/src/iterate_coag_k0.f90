...
subroutine iterate_coag_k0(path_data,path_log,chnbins,nbins,kpol,massgrid,massbins,Q,vecnodes,vecweights,ndthydro,dthydro)
   implicit none
   ...
   integer  :: j,i,iprogress,tot_nsub,tot_ndt,nsub,ndt
   ...
   call cpu_time(start)
   gij= 0._wp
   call L2proj_GQ_k0(eps,nbins,massgrid,massbins,Q,vecnodes,vecweights,gij)
   call cpu_time(finish)
   ...
   !write gij start and gij massmeanlog
   if (isave) then
   ...
   do i=1,ndthydro
      ...
      if (optimised_solver) then
         call compute_coag_k0_opt(eps,coeff_CFL,nbins,massgrid,massbins,tabflux_coag,dthydro,gij,gijnew,nsub,ndt)
      else
         call compute_coag_k0(eps,coeff_CFL,nbins,massgrid,tabflux_coag,dthydro,gij,gijnew,nsub,ndt)
      endif
